...
pub async fn add_sys_dept(req: &mut Request, res: &mut Response) {
    let item = req.parse_json::<AddDeptReq>().await.unwrap();
    ...
    match res_dept {
        Ok(r) => {
            if r.is_some() {
                return BaseResponse::<String>::err_result_msg(res, "部门名称已存在".to_string());
            }
        }
...
pub async fn delete_sys_dept(req: &mut Request, res: &mut Response) {
    let item = req.parse_json::<DeleteDeptReq>().await.unwrap();
    ...
    let res_dept = select_dept_count(&mut RB.clone(), &item.id)
        ...
        .unwrap_or_default();
    if res_dept > 0 {
        return BaseResponse::<String>::err_result_msg(res, "存在下级部门,不允许删除".to_string());
    }
    ...
    let res_dept = check_dept_exist_user(&mut RB.clone(), &item.id)
        ...
        .unwrap_or_default();
    if res_dept > 0 {
        return BaseResponse::<String>::err_result_msg(res, "部门存在用户,不允许删除".to_string());
    }
...
pub async fn update_sys_dept(req: &mut Request, res: &mut Response) {
    let item = req.parse_json::<UpdateDeptReq>().await.unwrap();
    log::info!("update sys_dept params: {:?}", &item);

    if item.parent_id == item.id {
        return BaseResponse::<String>::err_result_msg(res, "上级部门不能是自己".to_string());
    }
    ...
    if count > 0 && item.status == 0 {
        return BaseResponse::<String>::err_result_msg(res, "该部门包含未停用的子部门".to_string());
    }
    ...
    match res_dept {
        Ok(list) => {
            ...
            let result =
                Dept::update_by_column_batch(&mut RB.clone(), &depts, "id", depts.len() as u64)
                    .await;
            if result.is_err() {
                return BaseResponse::<String>::err_result_msg(
                    res,
...
pub async fn query_sys_dept_detail(req: &mut Request, res: &mut Response) {
    let item = req.parse_json::<QueryDeptDetailReq>().await.unwrap();
...
pub async fn query_sys_dept_list(req: &mut Request, res: &mut Response) {
    let item = req.parse_json::<QueryDeptListReq>().await.unwrap();
