...
pub async fn delete_sys_login_log(req: &mut Request, res: &mut Response) {
    let item = req.parse_json::<DeleteLoginLogReq>().await.unwrap();
...
pub async fn query_sys_login_log_detail(req: &mut Request, res: &mut Response) {
    let item = req.parse_json::<QueryLoginLogDetailReq>().await.unwrap();
...
pub async fn query_sys_login_log_list(req: &mut Request, res: &mut Response) {
    let item = req.parse_json::<QueryLoginLogListReq>().await.unwrap();
    ...
    let result =
        LoginLog::select_login_log_list(&mut RB.clone(), page, name, ipaddr, browser, os, &status)
            .await;
    ...
    match result {
        Ok(d) => {
            ...
            BaseResponse::<Vec<LoginLogListDataResp>>::ok_result_page(res, sys_login_log_list_data, total)
        }
        Err(err) => BaseResponse::<Vec<LoginLogListDataResp>>::err_result_page(res, err.to_string()),
    }
}
